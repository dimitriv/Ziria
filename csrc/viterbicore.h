...
*************************************************/
FINL
void Viterbi_sig11 (vub *pTrellis, const signed char * pbInput, char * pbOutput, int output_bit = 24)
{
    int i, j;
    ...
    const signed char * psbit = pbInput;     // pointer to the s-bit stream;
    ...
    while (i_trellis < output_bit ) {
        ...
        rub0 = bitwise_and ( rub0, ALLINVONE); // mark the path

        // branch 1
        rub1 = add ( rub1, VIT_MA[i_ma+1] );
        rub1 = add ( rub1, VIT_MB[i_mb+1] ); 
        rub1 = bitwise_or  ( rub1, ALLONE );
        ...
        if ((i_trellis & 7) == 0 ) {
            ...
            rub3 = bitwise_and  (rub3, ALLINVONE );
