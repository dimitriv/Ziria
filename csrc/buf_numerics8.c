...
unsigned int parse_dbg_uint8(char *dbg_buf, uint8 *target)
{
	char *s = NULL;
	unsigned int i = 0;
	unsigned long val;
...
void _init_getuint8(BlinkParams *params, BufContextBlock *blk, HeapContextBlock *hblk, size_t unit_size)
{
	blk->total_in = 0;
	...
	if (params->inType == TY_FILE)
	{
		...
		{
			unsigned int i;
			uint8 *typed_filebuffer = (uint8 *)filebuffer;
			blk->num8_input_entries = sz;					// We always count entries in ints and do two reads for complex
			for (i = 0; i < blk->num8_input_entries; i++)
			{
				blk->num8_input_buffer[i] = typed_filebuffer[i];
...
void init_getuint8(BlinkParams *params, BufContextBlock *blk, HeapContextBlock *hblk, size_t unit_size)
{
	blk->size_in = 8;
	// we just need to initialize the input buffer in the same way
	_init_getuint8(params, blk, hblk, unit_size);
}


FINL
GetStatus _buf_getuint8(BlinkParams *params, BufContextBlock *blk, uint8 *x)
{
	if (params->inType == TY_DUMMY)
	{
		if (blk->num8_input_dummy_samples >= blk->num8_max_dummy_samples && params->dummySamples != INF_REPEAT) return GS_EOF;
...
FINL
GetStatus _buf_getarruint8(BlinkParams *params, BufContextBlock *blk, uint8 *x, unsigned int vlen)
{
	if (params->inType == TY_DUMMY)
	{
		if (blk->num8_input_dummy_samples >= blk->num8_max_dummy_samples && params->dummySamples != INF_REPEAT) return GS_EOF;
	...
	if (params->inType == TY_FILE || params->inType == TY_MEM)
	{
		...
		memcpy(x, &(blk->num8_input_buffer[blk->num8_input_idx]), vlen * sizeof(uint8));
		blk->num8_input_idx += vlen;
		return GS_SUCCESS;
	}
...
void fprint_uint8(BufContextBlock *blk, FILE *f, uint8 val)
{
	if (blk->num8_fst == 1)
	{
		fprintf(f, "%u", val);
...
void fprint_arruint8(BufContextBlock *blk, FILE *f, uint8 *val, unsigned int vlen)
{
	unsigned int i;
	for (i = 0; i < vlen; i++)
	{
		fprint_uint8(blk, f, val[i]);
	}
}
...
FINL
void _buf_putuint8(BlinkParams *params, BufContextBlock *blk, uint8 x)
{
	if (params->outType == TY_DUMMY)
	{
		return;
	...
	if (params->outType == TY_MEM)
	{
		blk->num8_output_buffer[blk->num8_output_idx++] = (uint8)x;
	}

	if (params->outType == TY_FILE)
	{
		if (params->outFileMode == MODE_DBG)
			fprint_uint8(blk, blk->num8_output_file, x);
		else
		{
			if (blk->num8_output_idx == blk->num8_output_entries)
			{
				fwrite(blk->num8_output_buffer, blk->num8_output_entries, sizeof(uint8), blk->num8_output_file);
				blk->num8_output_idx = 0;
			}
			blk->num8_output_buffer[blk->num8_output_idx++] = (uint8)x;
		}
...
FINL
void _buf_putarruint8(BlinkParams *params, BufContextBlock *blk, uint8 *x, unsigned int vlen)
{
	...
	if (params->outType == TY_MEM)
	{
		memcpy((void*)(blk->num8_output_buffer + blk->num8_output_idx), (void*)x, vlen*sizeof(uint8));
		blk->num8_output_idx += vlen;
	}
	...
	{
		if (params->outFileMode == MODE_DBG)
			fprint_arruint8(blk, blk->num8_output_file, x, vlen);
		else
		{
			if (blk->num8_output_idx + vlen >= blk->num8_output_entries)
			{
...
void _flush_putuint8(BlinkParams *params, BufContextBlock *blk, size_t size)
{
	if (params->outType == TY_FILE)
	{
		if (params->outFileMode == MODE_BIN) {
			fwrite(blk->num8_output_buffer, sizeof(uint8), blk->num8_output_idx, blk->num8_output_file);
		}
	}
	blk->num8_output_idx = 0;
}
...
{
	_flush_putuint8(params, blk, sizeof(uint8));
	if (params->outType == TY_FILE)
	{
		fclose(blk->num8_output_file);
	}
