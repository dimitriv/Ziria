...
@echo msbuild CompilerVS13-mac.vcxproj /target:build /p:Configuration=Debug /p:Platform=x64 /m /verbosity:minimal
msbuild CompilerVS13-mac.vcxproj /target:build /p:Configuration=Debug /p:Platform=x64 /m /verbosity:minimal

if %errorlevel% NEQ 0 (
set errorlev=1
del CompilerVS13-mac\x64\Debug\CompilerVS13.exe 
) else (
set errorlev=0
)
exit /B %errorlev%
