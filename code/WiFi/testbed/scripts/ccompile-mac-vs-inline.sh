...
if [ "$MAC_TYPE" = 'TX' ]; then
  cp -f ../mac/CompilerVS/CompilerVS13-mac-inline/x64/Release/CompilerVS13-mac-inline.exe tx.out
else
  if [ "$MAC_TYPE" = 'RX' ]; then
    cp -f ../mac/CompilerVS/CompilerVS13-mac-inline/x64/Release/CompilerVS13-mac-inline.exe rx.out
  else
    if [ "$MAC_TYPE" = 'TEST_CCA' ]; then
      cp -f ../mac/CompilerVS/CompilerVS13-mac-inline/x64/Release/CompilerVS13-mac-inline.exe test_cca.out
    else
      cp -f ../mac/CompilerVS/CompilerVS13-mac-inline/x64/Release/CompilerVS13-mac-inline.exe mac.out
    fi
  fi
fi
