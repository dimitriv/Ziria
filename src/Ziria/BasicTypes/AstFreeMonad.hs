...
module Ziria.BasicTypes.AstFreeMonad (
...
import Ziria.Typecheck.Tc ( tc )
import Ziria.Utils.Rebindables
import Ziria.Typecheck.TcMonad
import Ziria.Typecheck.TcUnify (unify)
import Ziria.Typecheck (tyCheckComp)
import qualified Ziria.Utils.GenSym as GS

-- Imports for the example only:
-- import Ziria.BasicTypes.AstQuasiQuote (zcomp)
