...
module Ziria.Language.Ziria.Parser.Lexer (
    lexToken,
    lexTokens
  ) where
...
import Ziria.Language.Ziria.Parser.Alex
import Ziria.Language.Ziria.Parser.Monad
import Ziria.Language.Ziria.Parser.Tokens
}
...
identifier beg end =
    case Map.lookup ident keywords of
      Nothing  -> do isComp <- isCompIdentifier ident
                     if isComp
                        then token (TcompIdentifier ident) beg end
