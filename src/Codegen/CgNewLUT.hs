...
-- | Keep the low n bits of ce only
...
unpack_idx_var pkg v idx idx_ty
  ...
  = do base_w <- tyBitWidth base_ty
       ...
       appendStmt [cstm|bitArrWrite(idx_ptr,lidx*base_w,slice_w,vptr);|]
       let new_idx = idx `cExpShR` slice_w
       appendStmt $ [cstm| $idx = $new_idx;|]
  | otherwise
  = do w    <- inVarBitWidth pkg v
       vptr <- varToBitArrPtr v
       appendStmt [cstm|bitArrWrite((BitArrPtr) & $idx,0,w,varexp_ptr);|]
...
pack_out_var pkg (v,v_asng_mask) tgt pos = do
  ...
  appendStmt [cstm|bitArrWrite(vptr,$int:pos,w,$tgt)
  -- | Write the mask if there is one
  case v_asgn_mask of
    Just v' -> do mptr <- varToBitArrPtr v'
                  appendStmt [cstm|bitArrWrite(mptr,$int:pos+w,w,$tgt);|]
    Nothing -> return ()
  -- | Return new position
...
unpack_out_var pkg (v, Nothing) src pos = do
  ...
  return (pos+w)
unpack_out_var pkg (v, Just vm) src pos = do
  vptr    <- varToBitArrPtr v
  total_w <- outVarBitWidth pkg v
  let w  = total_w / 2
  vm_ptr <- varToBitArrPtr vm
...
cgBitArrRead_alg src pos len tgt
  ...
  | len == 24
  = do appendStmt [cstm| *((uint16 *) $tgt)   = *(uint16 *) (& $src[sidx]);  |]
       appendStmt [cstm| *((uint8 *) ($tgt+2) = *(uint8 *)  (& $src[sidx+2]);|]
  | len == 40
  = do appendStmt [cstm| *((uint32 *) $tgt)     = *(uint32 *) (& $src[sidx]);|]
       appendStmt [cstm| *((uint8 *) ($tgt + 4) = *(uint8 *) (& $src[sidx+4]);|]
  | len == 48
    do appendStmt [cstm| *((uint32 *) $tgt)      = *(uint32 *) (& $src[sidx]);  |]
       appendStmt [cstm| *((uint16 *) ($tgt + 4) = *(uint16 *) (& $src[sidx+4]);|]
  | len == 56
    do appendStmt [cstm| *((uint32 *) $tgt)      = *(uint32 *) (& $src[sidx]);  |]
       appendStmt [cstm| *((uint16 *) ($tgt + 4) = *(uint16 *) (& $src[sidx+4]);|]
  ...
  | otherwise
  = appendStmt [cstm| blink_copy((void *) $tgt, (void *) (& $src[sidx]);|]
