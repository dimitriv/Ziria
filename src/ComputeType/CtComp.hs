...
ctComp0 (Async _lc e)        = CTComp (TTask (ctExp e)) TVoid TVoid
ctComp0 (Await x)            = case (nameTyp x) of
                                 TTask t -> CTComp t TVoid TVoid
                                 _ -> panic $ text "ctComp: variable not of Task type"
