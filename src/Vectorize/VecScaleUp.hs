...
doVectCompUD dfs lcomp (SFUD1 i j (NMul m1) (NMul m2)) = do 
  venv <- getVecEnv
  let loc  = compLoc lcomp
      ioc  = runVecM (interpV loc $ vc (rewrite venv lcomp)) venv
      cty  = ctComp lcomp
      orig_inty  = inTyOfCTy cty
      orig_outty = yldTyOfCTy cty
      vin_ty  = mkVectTy orig_inty  (i*m1*m2)
      vout_ty = mkVectTy orig_outty (j*m1) 
  return $ DVR { dvr_comp = ioc, dvr_vres = DidVect vin_ty vout_ty minUtil }
  where
    rewrite :: VecEnv -> LComp -> EId -> EId -> EId -> EId -> EId -> IO Comp
    rewrite venv lc iidx oidx tidx xa ya
      = runVecM (rewriteTakeEmit on_take on_takes on_emit on_emits lc) venv
      where
        on_take loc ty = interpV loc $ do
          tidx  .:= iidx
          iidx  .:= iidx .+ I(1)
          freturn _fI $ xa .! tidx
        
        on_takes loc ty n = interpV loc $ do 
          tidx  .:= iidx 
          iidx  .:= iidx .+ I(n)
          freturn _fI $ xa .! (tidx :+ n)

        on_emit loc (e :: Exp) = interpV loc $ do 
          tidx .:= oidx
          oidx .:= oidx .+ I(1)
          ya .! tidx .:= e

        on_emits loc (es :: Exp) = interpV loc $ do 
          tidx .:= oidx
          oidx .:= oidx .+ I(len)
          ya .! (tidx :+ len) .:= es
    ...
    inty  = inTyOfCTy  (ctComp lcomp)
    outty = yldTyOfCTy (ctComp lcomp) 

    vc bd = do xa <- ftake (array_ty (i*m1*m2) inty)
               fleteref ("iidx" ::: tint ::= I(0)) $ \iidx ->
                fleteref ("tidx" ::: tint ::= I(0)) $ \tidx -> 
                 fleteref ("ya"   ::: array_ty (j*m1) outty) $ \ya ->
                  ftimes zERO m2 $ \_i -> 
                   fleteref ("oidx" ::: tint ::= I(0)) $ \oidx ->
                     do ftimes zERO m1 (\_ -> fembed (bd iidx oidx tidx xa ya))
                        oidx .:= zERO
                        femit ya 
    zERO = I(0) 
{- 
...
-}
doVectCompUD dfs lcomp _ = error "implementme" 
