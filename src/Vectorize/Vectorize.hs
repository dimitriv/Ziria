...
allVectMults ty_in xin xout = [ (x,y) 
                              | x <- [1..64]
                              , y <- [1..64]
                              , x `mod` y == 0 && x >= y
                              , good_sizes x y 
...
mitigateDn :: Maybe SourcePos
           -> Ty -> Int -> Int -> VecMBnd (Comp () ())
mitigateDn loc ty hi lo 
  = do { x <- newTypedName "x" (TArr (Literal hi) ty) loc
       ; i <- newTypedName "i" tint loc
       ; let bnd = hi `div` lo
...
matchControl (sym,venv) bcands 
  ...
  where 
    ...
    go acc ((c1:c1s):crest) k 
      = go (acc ++ [c1]) (c1s:crest) (go acc (c1s:crest) k)
    go acc [] k                 
      = acc : k
    ...
    mitigate_one ain aout 
      ...
      = let mk_in c 
        ...
        let mk_out c
              ...
              , let loc = compLoc c
              = runVecMBnd $ 
                do { m <- mitigateDn loc tout cout aout
                   ; return $ 
...
mitigatePar (sym,venv) pnfo loc dp1 dp2
 = case (dvr_vres dp1, dvr_vres dp2) of
  ...
  where mk_par Nothing dp1 dp2 
        ...
        mk_par (Just (ty,hi,lo)) dp1 dp2
          = let vec_action 
                   = do { (m,binds) <- runVecMBnd $ mitigateDn loc ty hi lo
                        ; p1 <- vecMIO (dvr_comp dp1)
                        ; p2 <- vecMIO (dvr_comp dp2)
                        ; let p1'  = cPar loc () (mkParInfo NeverPipeline) p1 m 
                              comp = cPar loc () pnfo p1' p2
